...
#import "BROWNIEViewController.h"

@interface BROWNIEViewController (){
...
@implementation BROWNIEViewController
...
- (IBAction)brownie:(id)sender {
    ...
    double valor = [_stepper_brownie value];
    
    [_brownie setText:[NSString stringWithFormat:@"%d", (int)valor]];
...
- (IBAction)carritoFinal:(id)sender {
    ...
    int brownie = [self.brownie.text intValue];
    ...
    if (sqlite3_open([dbPathString UTF8String], &suiker_carrito)==SQLITE_OK) {
        NSString *insertStmt = [NSString stringWithFormat:@"UPDATE SUIKERFINAL SET BROWNIE = BROWNIE + %d", brownie];
