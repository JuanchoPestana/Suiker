...
#import "BOTE_PVCViewController.h"

@interface BOTE_PVCViewController (){
...
@implementation BOTE_PVCViewController
...
- (IBAction)carritoFinal:(id)sender {
    ...
    if (sqlite3_open([dbPathString UTF8String], &suiker_carrito)==SQLITE_OK) {
        NSString *insertStmt = [NSString stringWithFormat:@"UPDATE SUIKERFINAL SET BOTE_PVC_CHICO = BOTE_PVC_CHICO + %d, BOTE_PVC_MEDIANO = BOTE_PVC_MEDIANO + %d, BOTE_PVC_GRANDE = BOTE_PVC_GRANDE + %d", chico, mediano, grande];
